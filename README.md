...
Visual Dialogue Portrait email development is mainly split into two programs:
...
Since some email clients do not support CSS defined in the `<style>` tags, Visual dialog has made several options to move all the css inline when compiling. This can be useful or detrimental depending on how you apply it.
...
The class does not need to be placed into any master template to run, as it already
...
There is much more to email development than what i have written here. Feel free to correct any mistakes in this repository and add new information to it.
